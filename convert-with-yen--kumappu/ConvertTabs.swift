...
struct ConvertTabs: View {
    @Binding var activeTab: ConvertTab
    
    @Namespace private var namespace
    let tabWidth: CGFloat = 172

    var body: some View {
        HStack(spacing: 0) {
            ForEach(ConvertTab.allCases, id: \.self) { tab in
                Button(action: {
...
#Preview {
    ConvertTabs(activeTab: .constant(.convertTo))
}
